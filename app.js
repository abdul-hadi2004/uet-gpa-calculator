...
(function () {
    ...
    function renderImprovementAdvice(advice) {
        ...
        if (top) {
            html += '<div class="advice-priority"><strong>Improvement Priority</strong>' +
                '<span class="advice-item">' + esc(top.name) + ' — ' + top.credits + ' ' + (top.credits === 1 ? 'credit hour' : 'credit hours') + ' — Grade ' + esc(top.grade) + ' (potential gain: +' + top.potentialGain.toFixed(2) + ' QP)</span></div>';
        }
        if (advice.lowest) {
            html += '<div class="advice-lowest"><strong>Lowest Grade</strong>' +
                '<span class="advice-item">' + esc(advice.lowest.name) + ' — Grade ' + esc(advice.lowest.grade) + ' (' + advice.lowest.gradePoint.toFixed(2) + ' grade points)</span></div>';
        }
        box.innerHTML = html;
    }
    ...
    function onCalcSgpa() {
        ...
        rows.forEach(function (row) {
            var res = validateRow(row);
            if (!res.valid) { hasInvalid = true; showRowErrors(row, res.errors); }
            else { clearRowErrors(row); updateRowPoints(row); }
        });
        if (hasInvalid) { toast('Please correct the highlighted errors before calculating.', 'error'); return; }
    ...
    function onCalcCgpa() {
        ...
        var hasInvalid = false;
        rows.forEach(function (row) { if (!validateSemesterRow(row).valid) hasInvalid = true; });
        if (hasInvalid) { toast('Please correct the highlighted errors before calculating.', 'error'); return; }
